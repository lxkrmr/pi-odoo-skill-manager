...
"$PY" "$CLI" help --output json | "$PY" -c 'import json,sys; d=json.load(sys.stdin); assert d["ok"]; data=d["data"]; assert data["contract_version"]=="v1"; assert data["contract_spec"]=="docs/cli-contract.md"; nc=set(data["non_contract_commands"]); assert {"new-skill","up","db","shell","test","lint","ui"}.issubset(nc); details={x["name"]:x for x in data["details"]}; assert details["wizard"]["contract_scope"]=="automation"; assert details["db"]["contract_scope"]=="human-ops"'
